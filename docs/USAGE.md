...
当前任务菜单显示剩余业务轮数；结果反馈和最终验收也计入预算，剩余一轮时会提醒预留给最终验收。此提示不自动续批，也不改变既有暂停和确认规则。

任务菜单与 `status.lastReply` 保留最近一次有效回复的简短统计：`elapsedMs` 是从该业务轮登记到有效回复校验通过的总耗时（包含分段读取、格式修复和用户暂停期间，不含后续工具执行或清理），`repairCount` 是该轮已发送的格式修复次数，`round` 是当批轮次。等待新回复时仍显示上一次已校验结果；正常收尾后可继续查看。统计只保留一份于内存，归属原任务，不含消息正文，插件重载后清空。
...
默认提示词要求规划端在材料不足时索取具体文件、diff 或测试输出，依据反馈推进，证据不足时明确缺口。每条业务消息都告知本批剩余消息数，包含最终反馈。自然语言回复的完整步骤仅保留在 `actions[].instruction`，`summary` 是短预览；代码块中的状态示例不作为额外控制指令。回复长度会计入 Windows JSON 转义开销，超限报 `RESULT_TOO_LARGE`，不截断执行步骤或自动重发。
